...
def clean_currency_vdu(x):
    """Limpieza robusta para evitar desplazamientos de comas y puntos."""
    if not x or str(x).strip() == "" or str(x).lower() in ["nan", "null", "none", "-"]: 
        return 0.0
    cleaned = str(x).replace('$', '').replace(' ', '')
    
    # Si tiene ambos separadores (1.250,50)
    if ',' in cleaned and '.' in cleaned:
        cleaned = cleaned.replace('.', '').replace(',', '.')
    # Si tiene coma pero parece ser separador de miles (1,250)
    elif ',' in cleaned:
        partes = cleaned.split(',')
        if len(partes[-1]) <= 2: # Probable decimal
    ...
    try:
        ...
        def get_cubo_data(book_id):
            try:
                ...
                # Mapeo de seguridad para nombres de columnas
                ...
                st.warning(f"Aviso: No se pudo procesar 'Cubo' en {book_id}: {e}")
                return pd.DataFrame()
        ...
        if df_s.empty: return pd.DataFrame(), df_u
        
        # 4. Limpieza de valores numéricos
        for col in ['coin_in', 'win', 'jackpot']:
            if col in df_s.columns:
                df_s[col] = df_s[col].apply(clean_currency_vdu)
...
if df_users is not None:
    try:
        ...
        st.error(f"Error en estructura de usuarios: {e}")
        st.stop()
    ...
    if st.session_state.get("authentication_status"):
        ...
        if nav == "📊 Dashboard de Sala":
            ...
            if df_slots is not None and not df_slots.empty:
                ...
                df_ev = df_f.groupby('fecha')[['win', 'coin_in']].sum().reset_index()
                st.plotly_chart(px.area(df_ev, x='fecha', y=['win', 'coin_in'], 
                                        template="plotly_dark", 
                                        color_discrete_map={'win': '#00D1FF', 'coin_in': '#FF4B4B'},
                                        title="Evolución de Rendimiento"), use_container_width=True)
                ...
                st.error("Error: No se pudieron consolidar los datos de las pestañas 'Cubo'.")

        elif nav == "🔄 Analista Comparativo":
            st.title("⚖️ Diagnóstico Comparativo")
            st.info("Compare el rendimiento entre dos periodos para detectar variaciones de rentabilidad.")
            ...
            if not df_slots.empty:
                ...
                if len(r_actual) == 2 and len(r_previo) == 2:
                    ...
                    st.divider()
                    st.subheader("Análisis de Desviación por Asset")
                    df_diff = df_a.groupby('asset_Id')['win'].sum().reset_index()
                    df_diff_b = df_b.groupby('asset_Id')['win'].sum().reset_index()
                    df_merged = pd.merge(df_diff, df_diff_b, on='asset_Id', suffixes=('_A', '_B'))
                    df_merged['Diferencia'] = df_merged['win_A'] - df_merged['win_B']
