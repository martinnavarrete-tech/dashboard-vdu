...
def render_auditor_cuadros(df):
    """Genera los 4 cuadros del Auditor Interno dinámicos"""
    if df.empty: return
    ...
    with c3:
        ...
        st.markdown(f"""<div class='metric-card' style='border-bottom-color: {color}'>
            ...
            <div class='metric-sub'>Máquinas con pérdida neta en el periodo.</div>
        </div>""", unsafe_allow_html=True)

    # 4. Volumen de Juego
    with c4:
        st.markdown(f"""<div class='metric-card'>
            <div class='metric-label'>VOLUMEN DE JUEGO</div>
            <div class='metric-value'>{form_num(t_ci)}</div>
            <div class='metric-sub'>Coin In acumulado con filtros activos.</div>
        </div>""", unsafe_allow_html=True)
    ...
    if st.session_state.get("authentication_status"):
        ...
        if nav == "📊 Dashboard":
            ...
            with tab_excep:
                ...
                if inactive:
                    st.warning(f"Se detectaron {len(inactive)} máquinas sin movimiento en el periodo y filtros actuales.")
                    st.write(inactive)
                else:
                    st.success("Todos los activos seleccionados registraron actividad.")
            ...
            with st.container(border=True):
                ...
                with col1:
                    r_a = st.date_input("Periodo A (Actual/Referencia)", [df_slots['fecha'].max() - timedelta(days=7), df_slots['fecha'].max()])
                with col2:
                    r_b = st.date_input("Periodo B (Comparación/Anterior)", [df_slots['fecha'].max() - timedelta(days=15), df_slots['fecha'].max() - timedelta(days=8)])
                
                f_comp_marca = st.multiselect("Filtrar comparación por Marca", sorted(df_slots['marca'].unique()))
            ...
            if len(r_a) == 2 and len(r_b) == 2:
                ...
                elif pct_w > 0 and pct_c < 0:
                    st.info("Alerta de Hold: El Win subió a pesar de que hay menos juego; esto indica una subida atípica en la retención de las máquinas.")
                elif pct_w < 0 and pct_c > 0:
