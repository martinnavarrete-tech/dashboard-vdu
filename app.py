    ...
    if st.session_state.get("authentication_status"):
        ...
        if nav == "📊 Dashboard de Sala":
            ...
            with tab1:
                ...
                with h1:
                    st.markdown(f"<div class='metric-card' style='border-left:5px solid #00D1FF;'><div class='metric-label'>💎 Dominio Win</div><div class='metric-value'>{lider_row[col_analisis]}</div><div class='metric-sub'>Representa el <b>{share_win:.1f}%</b> del Win Total {contexto}.</div></div>", unsafe_allow_html=True)
                with h2:
                    st.markdown(f"<div class='metric-card' style='border-left:5px solid #BB86FC;'><div class='metric-label'>🔥 Imán de Tráfico</div><div class='metric-value'>{lider_coin_row[col_analisis]}</div><div class='metric-sub'>Mueve el <b>{share_coin:.1f}%</b> del Coin In {contexto}.</div></div>", unsafe_allow_html=True)
                with h3:
                    st.markdown(f"<div class='metric-card' style='border-left:5px solid #00FF88;'><div class='metric-label'>🚀 Máxima Eficiencia</div><div class='metric-value'>{efi_row.name}</div><div class='metric-sub'>Rinde <b>{performance_vs_avg:.1f}x</b> más que el promedio de sala.</div></div>", unsafe_allow_html=True)
                with h4:
                    st.markdown(f"<div class='metric-card' style='border-left:5px solid #FF4B4B;'><div class='metric-label'>⚠️ Lucro Cesante</div><div class='metric-value'>{len(ociosas)} Assets</div><div class='metric-sub'><b>{porcentaje_ociosas:.1f}%</b> de la flota analizada sin actividad física.</div></div>", unsafe_allow_html=True)
